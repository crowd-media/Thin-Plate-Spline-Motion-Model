...
class VideoCropper():
    def __init__(inp, cpu = False, image_shape = (256, 256), increase = 0.1, iou_with_initial = 0.25, min_frames = 150):
        self.inp = inp
        self.cpu = cpu
    ...
    def compute_bbox_trajectories(self, trajectories, fps, frame_shape, args):
        commands = []
        for i, (bbox, tube_bbox, start, end) in enumerate(trajectories):
            if (end - start) > self.min_frames:
                command = compute_bbox(start, end, fps, tube_bbox, frame_shape, inp=self.inp, image_shape=self.image_shape, increase_area=self.increase)
    ...
    def process_video(self):
        ...
        video = imageio.get_reader(self.inp)
